...
export default function HorizontalMenu(props: props) {
    ...
    const getMenuOpts = () => {
        let list = []
        list[0] = (
            <div key={0} className="flex items-center justify-center hover:bg-gray-200 hover:dark:bg-gray-800 rounded-3xl w-fit px-2" onClick={() => nav(opts2[0].toLowerCase())}>
                <GrHomeRounded size={20} className="text-white" />
            </div>

        )
        list[1] = (
            <div key={1} className="flex items-center hover:bg-gray-200 hover:dark:bg-gray-800 rounded-3xl w-fit px-2" onClick={() => nav("/" + username)}>
                <HiOutlineUser size={20} className="text-white" />
            </div>
        )
        list[2] = (
            <div key={2} className=" relative flex items-center rounded-3xl hover:bg-gray-200 hover:dark:bg-gray-800 w-fit px-2" onClick={() => nav("/notifications", { state: { id: id, username: username } })}>
